...
**Почему Next.js?**
App Router решил сразу две задачи: встроенный API proxy через route handlers позволил инжектировать анонимный `user-id` из HttpOnly cookie на сервере — без отдельного auth-сервиса и без утечки идентификатора в клиентский код. Кроме того, route handler прозрачно пробрасывает SSE-поток от бэкенда напрямую в браузер через `ReadableStream`, не буферизируя ответ — что критично для стриминга.
...
**Почему Bun?**
Bun — это рантайм, бандлер и пакетный менеджер в одном. Для этого проекта критично то, что в него встроен драйвер SQLite (`bun:sqlite`) — не нужно тащить нативные C-биндинги через `better-sqlite3` или поднимать отдельный процесс. Всё работает из коробки, бинарник один, Docker-образ минимальный. Скорость холодного старта у Bun заметно выше Node.js, что важно при контейнерном деплое.

**Почему Fastify?**
Express удобен, но он буферизует тело ответа — это смерть для SSE. Fastify даёт доступ к голому Node.js `http.ServerResponse` через `reply.raw`, что позволяет писать в сокет напрямую (`write`, `end`) без какой-либо буферизации на уровне фреймворка. Это гарантирует, что каждый токен от OpenRouter долетает до браузера немедленно, а не после накопления буфера. Помимо этого, Fastify имеет встроенную схему валидации через JSON Schema — меньше бойлерплейта для валидации входящих данных.
